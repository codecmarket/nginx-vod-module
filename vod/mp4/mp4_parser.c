...
// computes the peak bitrate: the max bitrate over key-frame aligned windows of at least segment_duration
static vod_status_t
		...
		if (uniform_size == 0)
		{
			...
			default:
				vod_log_debug1(VOD_LOG_DEBUG_LEVEL, context->request_context->log, 0,
					"mp4_parser_compute_bitrate: unsupported field_size %uD", field_size);
				return VOD_BAD_DATA;
			}
		}
...
mp4_parser_process_moov_atom_callback(void* ctx, atom_info_t* atom_info)
{
	...
	// inherit bitrate from sequence (zero means: compute from stream data)
	...
	media_track_array_t* result)
{
	...
	for (cur_track = first_track; cur_track < last_track; cur_track++)
	{
		...
		{
			// estimate the average bitrate from frame size when not resolved
			if (cur_track->media_info.avg_bitrate == 0)
			{
				cur_track->media_info.avg_bitrate = (uint32_t)((context.total_frames_size * cur_track->media_info.timescale * 8) / cur_track->media_info.full_duration);
			}

			// compute the peak bitrate from stream data when not resolved
			if (cur_track->media_info.bitrate == 0)
			{
				...
				// fallback to the average bitrate if unresolved
				if (cur_track->media_info.bitrate == 0)
				{
